    ...
    def gradient_step(self):
        if self.buffer.size > self.batch_size:
            ...
            if self.distributional:
                self.loss_distributional(state, action, next_state, reward, done) 
                if self.noisy:
...
model = RainbowNet(config['obs_space'], 256, config['nb_actions'], n_atoms, 4, support, config['noisy'], dueling=config['dueling'])
