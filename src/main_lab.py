...
def main(env, replay_buffer, is_atari, state_dim, num_actions, args, parameters, device):
	...
	elif args.algorithm == "GER_DDQN":
		policy = GER_DDQN.GER_DDQN(**kwargs)
	elif args.algorithm == "LABER_DDQN":
		policy = LABER_DDQN.LABER_DDQN(**kwargs)
	...
	for t in range(int(args.max_timesteps)):
		...
		if done or trunc:
			...
			print(f"Total T: {t+1} Episode Num: {episode_num+1} Eps: {max(policy.slope * policy.iterations + policy.initial_eps, policy.end_eps)} Reward: ", locale.format_string('%d', int(episode_reward), grouping=True))
			# Reset environment
			score_agent: float = evaluate_HIV(agent=policy, nb_episode=1)
			print(locale.format_string('%d', int(score_agent), grouping=True))
			if score_agent > best_reward:
				path = args.algorithm + "_best_model.pt"
				torch.save({'model_state_dict': policy.Q.state_dict(),
                    'reward': score_agent,}, path)
...
def eval_policy(policy, env_name, seed, eval_episodes=1):
	...
	for _ in range(eval_episodes):
		state, trunc = eval_env.reset()[0], False
		tab_actions = [0,0,0,0]
		while not trunc:
			action = policy.select_action(np.array(state), eval=True)
			tab_actions[action] += 1
			state, reward, done, trunc, _ = eval_env.step(action)
			avg_reward += reward
		print("Recap actions in eval :", tab_actions)
	avg_reward /= eval_episodes

	print("---------------------------------------")
	print(f"Evaluation over {eval_episodes} episodes: {locale.format_string('%d', int(avg_reward), grouping=True)}")
	print("---------------------------------------")
	return avg_reward
...
if __name__ == "__main__":
	...
	parser.add_argument("--algorithm", default="LABER_DDQN")		    # OpenAI gym environment name
	...
	# Initialize buffer
	prioritized = True if args.algorithm == "PER_DDQN" or args.algorithm == "GER_DDQN" else False
