...
class GER_DDQN(object):
    ...
    def train(self, replay_buffer):
        ...
        td_errors = td_errors.cpu().detach().numpy().flatten()
        ...
        for i in range(batch_size):
            ...
            for t in grads[i]:
                t_temp = t.clone()
                grads_i.append(t_temp.cpu().detach().numpy().flatten())
            grads_i = np.array(grads_i, dtype=object)  # Specify dtype=object here
            l_i = np.concatenate([g for g in grads_i])  # Make sure all arrays can be concatenated
