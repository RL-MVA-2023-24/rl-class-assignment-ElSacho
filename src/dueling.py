...
if config['dueling']:
    model = DuelingModel(config['obs_space'], 256, config['nb_actions'], 6).to(device)
elif not config['noisy']:
    # model = DQM_model(6, 256, 4, 6).to(device)
    model = DQM_model(config['obs_space'], 256, config['nb_actions'], 6).to(device)
else :
    model = Network(config['obs_space'], config['nb_actions'], config['noisy_std_init']).to(device)
# Train agent
