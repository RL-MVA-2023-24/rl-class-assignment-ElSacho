...
class dqn_agent:
        ...
        while episode < max_episode:
            ...
            if self.update_target_strategy == 'ema':
                ...
                for key in model_state_dict:
                    # Ensure the operation is applied to the parameter values, not the dictionaries
                    target_state_dict[key] = tau * model_state_dict[key] + (1 - tau) * target_state_dict[key]
                # After updating the target_state_dict, load the updated state into the target model
    ...
    def save(self, path = "noisy_normalise.pt"):
        torch.save({
                    'model_state_dict': self.model.state_dict()
                    }, path)
...
class MSLELoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        y_pred_adjusted = torch.max(y_pred, torch.tensor(1.0))  
        y_true_adjusted = torch.max(y_true, torch.tensor(1.0))  
        return torch.mean((torch.log(y_pred_adjusted) - torch.log(y_true_adjusted)) ** 2)
