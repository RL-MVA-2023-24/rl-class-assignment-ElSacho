...
from models import DQM_model, Network, DuelingModel, DistribNet
...
class dqn_agent:
    ...
    def gradient_step(self):
        if self.buffer.size > self.batch_size:
            ...
            if self.distributional:
                self.loss_distributional(state, action, next_state, reward, done) #TODO : ADD PER
                if self.noisy:
                    self.model.reset_noise()
                    self.target_model.reset_noise()
            elif self.double:
                self.compute_loss_double(state, action, next_state, reward, done, ind, weights) #TODO : ADD PER
                if self.noisy:
                    self.model.reset_noise()
                    self.target_model.reset_noise()
            else :
    ...
    def loss_distributional(self, states, actions, next_states, rewards, dones):
...
if config['distributional']:
    ...
    model = DistribNet(config['obs_space'], config['nb_actions'], n_atoms, support).to(device)
elif config['dueling']:
