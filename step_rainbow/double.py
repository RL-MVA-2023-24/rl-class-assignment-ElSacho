...
from models import DQM_model, Network
...
class dqn_agent:
    ...
    def save(self, path = "d=t_r=t_n=t.pt"):
        torch.save({
                    'model_state_dict': self.model.state_dict()
                    }, path)
...
if not config['noisy']:
    ...
    model = DQM_model(config['obs_space'], 256, config['nb_actions'], 6).to(device)
else :
    model = Network(config['obs_space'], config['nb_actions'])
# Train agent
...
# agent = dqn_agent(config, model)
# ep_return2 = agent.train(env, 200)
...
# agent = dqn_agent(config, model)
# ep_return3 = agent.train(env, 200)

# # Plotting the data
# plt.figure(figsize=(10, 6))
# plt.plot(ep_return, label='Double', marker='o')
# plt.plot(ep_return2, label='Prioritized', marker='s')
# plt.plot(ep_return3, label='None', marker='^')
