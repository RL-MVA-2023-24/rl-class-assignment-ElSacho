...
class dqn_agent:
    ...
    def train(self, env, max_episode):
        ...
        obs_space = 6
        states = tab_state(obs_space)
        step_ep = 0
        state, _ = env.reset()
        states.push(state)
        action = 0
        next_state, reward, done, trunc, _ = env.step(action)
        state = states.push(next_state)
        ...
        while episode < max_episode:
            ...
            next_state, reward, done, trunc, _ = env.step(action)
            # state *= 1e-3
            next_state_norm = states.push(next_state)
            # self.memory.append(last_states, action, reward, states.tab_state, done)
    
            self.memory.append(state, action, reward, next_state_norm, done)
            ...
            if done or trunc:
                ...
                action = 0
                next_state, reward, done, trunc, _ = env.step(action)
                state_norm = states.push(next_state)
                episode_cum_reward = 0
            else:
