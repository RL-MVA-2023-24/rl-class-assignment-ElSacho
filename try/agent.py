...
class Agent:
    ...
    def _build_model(self):
        # Define input layer with the fixed shape of state_size
        inputs = Input(shape=(self.state_size,))
        
        # Fully connected layers replacing Conv2D layers
        x = Dense(32, activation='relu')(inputs)
        x = Dense(64, activation='relu')(x)
        x = Dense(64, activation='relu')(x)
        
        # Dueling Network architecture
        # Separate dense layer for the value stream
        val = Dense(1, activation='linear')(x)
        # Separate dense layer for the advantage stream
        advantage = Dense(self.action_size, activation='linear')(x)
        
        # Compute the mean of the advantage
        mean = Lambda(lambda a: K.mean(a, axis=1, keepdims=True))(advantage)
        advantage = Subtract()([advantage, mean])
        
        # Combine value and advantage into the final output
    ...
    def replay(self, batch_size):
        ...
        for state, action, reward, next_state, done in batch:
            ...
            if not done:
                # Double DQN
                m_a = np.argmax(self.model.predict(next_state)[0])
                target = (reward + self.gamma * self.target_model.predict(next_state)[0][m_a])
            else:
                target = reward
    ...
    def calculate_td_error(self, state, action, reward, next_state, done):
        ...
        target_f = self.model.predict(state)[0][action]
