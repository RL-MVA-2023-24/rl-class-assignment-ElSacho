...
class M_DQN_Agent():
    ...
    def __init__(self,
        ...
        self.qnetwork_local = MLP_DQN(6, 256, 4, depth=5).to(device)
        self.qnetwork_target = MLP_DQN(6, 256, 4, depth=5).to(device)
        if False:
            checkpoint = torch.load("test.pt", map_location=torch.device('cpu'))
...
def run(frames=1000, eps_fixed=False, eps_frames=1e6, min_eps=0.01):
    ...
    for frame in range(1, frames+1):
        ...
        if done or trunc:
            ...
            print('\rEpisode {}\tFrame {} \tAverage Score: {}'.format(i_episode, frame, locale.format_string('%d', int(average), grouping=True)), end="")
            if i_episode % 100 == 0:
                print('\rEpisode {}\tFrame {}\tAverage Score: {}'.format(i_episode,frame, locale.format_string('%d', int(average), grouping=True)))
            i_episode +=1 
            state, _ = env.reset()
            score = 0              
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    # final_average100 = run(frames = 450_000, eps_fixed=eps_fixed, eps_frames=50_000, min_eps=0.025)
    final_average100 = run(frames = 45_000, eps_fixed=eps_fixed, eps_frames=5_000, min_eps=0.025)
    t1 = time.time()
    
    print("Training time: {}min".format(round((t1-t0)/60,2)))
    torch.save(agent.qnetwork_local.state_dict(), "M-DQN_03"+".pth")
