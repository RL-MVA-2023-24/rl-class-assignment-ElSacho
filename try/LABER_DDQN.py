...
class LABER_DDQN(object):
    def __init__(
        ...
        self.Q = DQM_model(6, 256, 4, 6).to(device)
        self.Q.load_state_dict(torch.load("MDP.pt")['model_state_dict'])
        print("MOdel Loaeded")
    ...
    def select_action(self, state, eval=False):
        ...
        if np.random.uniform(0,1) > eps:
            with torch.no_grad():
                ...
                # return self.Q(state).argmax(1, keepdim=True)
    ...
    def train(self, replay_buffer):
        ...
        with torch.no_grad():
            ...
            td_errors_temp = (current_Q - target_Q).abs()
            td_errors = td_errors_temp.clone()
            ...
            loss_weights = torch.from_numpy(loss_weights).unsqueeze(1).to(self.device)


        # Compute Q loss for the mini-batch selected
        Q_current = self.Q(states_selected).gather(1, actions_selected)
        # print(f"Le tensor est sur: {Q_current.device}")
        # print(f"Le tensor est sur: {Q_targets.device}")
    ...
    def act(self, observation, use_random=False):
        return self.select_action(observation, eval=True)
        return greedy_action(self.model, observation)
    
    def save(self, path = "laber.pt"):
        torch.save({
    ...
    def load(self):
        path = "test.pt"
        self.model = DQM_model(6, 256, 4, 6).to(self.device)
